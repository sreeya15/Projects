...
*                    2. Periodically publishes temperature readings from a DHT sensor to the "home/room1/temp" MQTT topic.
...
const char* mqtt_server = "192.168.41.11";  // Change to your MQTT Raspberry Pi broker IP
...
void callback(char* topic, byte* payload, unsigned int length) {
  Serial.print("Message arrived [");  // Print a message indicating that a message has arrived.
  ...
  if (strcmp(topic, "home/rooms") == 0) {  // Check if the received topic is "home/rooms."
    if ((char)payload[0] == '1') {         // Check if the first character of the payload is '1.'
      ...
      digitalWrite(LIGHT1, HIGH);  // Turn off LED1 by making the voltage HIGH.
      digitalWrite(LIGHT2, HIGH);  // Turn off LED2 by making the voltage HIGH.
    }
  } else if (strcmp(topic, "home/room1/led") == 0) {  // Check if the received topic is "home/room1/led."
    if ((char)payload[0] == '1') {                    // Check if the first character of the payload is '1.'
      digitalWrite(LIGHT1, LOW);                      // Turn on LED1 by setting its voltage level LOW.
      digitalWrite(LIGHT2, LOW);                      // Turn on LED2 by setting its voltage level LOW.
    } else {
  ...
  } else if (strcmp(topic, "home/room1/light1") == 0) {  // Check if the received topic is "home/room1/light1."
    if ((char)payload[0] == '1') {                       // Check if the first character of the payload is '1.'
      digitalWrite(LIGHT1, LOW);                         // Turn on LED1 by setting its voltage level LOW.
    } else {
      digitalWrite(LIGHT1, HIGH);  // Turn off LED1 by making the voltage HIGH.
    }
  } else if (strcmp(topic, "home/room1/light2") == 0) {  // Check if the received topic is "home/room1/light2."
    if ((char)payload[0] == '1') {                       // Check if the first character of the payload is '1.'
      digitalWrite(LIGHT2, LOW);                         // Turn on LED2 by setting its voltage level LOW.
    } else {
      digitalWrite(LIGHT2, HIGH);  // Turn off LED2
    }
  }
}
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {  // Check if the connection attempt was successful.
      ...
      // Publish a temperature message to the MQTT topic "home/room1/temp."
      client.publish("home/room1/temp", "Temperature");

      // Resubscribe to specific MQTT topics
      client.subscribe("home/#");         // Subscribe to the "home/rooms" topic.
     // client.subscribe("home/room1/led");     // Subscribe to the "home/room1/led" topic.
      //client.subscribe("home/room1/light1");  // Subscribe to the "home/room1/light1" topic.
      //client.subscribe("home/room1/light2");  // Subscribe to the "home/room1/light2" topic.
    } else {
...
void loop() {
  ...
  if (now - lastMsg > 3000) {                                         // Check if it's been more than 3 seconds since the last message was sent
    ...
    client.publish("home/room1/temp", String(temperatureC).c_str());  // Publish the temperature value to the MQTT topic "home/room1/temp"
  }
}